...
document.addEventListener('DOMContentLoaded', () => {
  ...
  numbers.forEach(x =>{
    x.addEventListener('click', clickNumber);
  });
  ...
  document.querySelector ('button.decimal').addEventListener('click', clickNumber);
  // get a reference to the all clear button
  // add a click event listener to call the function clear  
  document.querySelector ('button.all-clear').addEventListener('click', clear);
  ...
  operators.forEach (x => {
    x.addEventListener('click', clickOperator);
  });
  // add click event listener for the equal sign
  // should call the function performOperation
  document.querySelector('button.equal-sign').addEventListener('click', performOperation);
});
